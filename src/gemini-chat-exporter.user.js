    ...
    const MarkdownParser = {
        parseNode: function(node) {
            ...
            switch (node.nodeName) {
                ...
                case "PRE":
                    const lang = node.closest('code-block')?.querySelector('span')?.textContent || "";
                    return `\n\`\`\`${lang}\n${node.textContent.trim()}\n\`\`\`\n`;
                case "UL": return `\n${this.parseChildren(node)}\n`;
                case "OL":
                    let olText = "\n";
                    Array.from(node.children).forEach((li, i) => olText += `${i+1}. ${this.parseChildren(li).trim()}\n`);
                    return olText;
